...
if (APP_CONFIG.VERSION !== 'v.1.8.8') {
  console.error('[ERRO] Versão diferente de v.1.8.8');
  process.exit(1);
}
...
if (pkg.version !== '1.8.8' || !/^[0-9]\.[0-9]\.[0-9]$/.test(pkg.version)) {
...
if (!indexHtml.includes('v.1.8.8')) {
  console.error('[ERRO] index.html não contém v.1.8.8');
  process.exit(1);
}
...
console.log('[OK] index.html contém v.1.8.8, novo cabeçalho Open Mark com logo SVG e badge Tamanho do MD');
...
if (!readme.includes('v.1.8.8')) {
  console.error('[ERRO] README.md não contém v.1.8.8');
  process.exit(1);
}
...
console.log('[OK] README.md contém cabeçalho v.1.8.8, título # Open Mark e link de acesso online imediato');
