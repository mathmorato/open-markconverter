...
if (APP_CONFIG.VERSION !== 'v.1.8.1') {
  console.error('[ERRO] Versão diferente de v.1.8.1');
  process.exit(1);
}
...
if (pkg.version !== '1.8.1' || !/^[0-9]\.[0-9]\.[0-9]$/.test(pkg.version)) {
...
if (!indexHtml.includes('v.1.8.1')) {
  console.error('[ERRO] index.html não contém v.1.8.1');
  process.exit(1);
}
...
console.log('[OK] index.html contém v.1.8.1, novo cabeçalho Open Mark com logo SVG e barra global de progresso');
...
console.log('[OK] js/app.js contém pool dinâmico de até 1000 workers e batchAnimationController com requestAnimationFrame');
...
console.log('[OK] css/styles.css contém layout travado de cabeçalho (CLS=0), estilos da barra com aceleração GPU e dropzone');

// 6. Verifica README.md
const readme = fs.readFileSync('./README.md', 'utf8');
if (!readme.includes('v.1.8.1')) {
  console.error('[ERRO] README.md não contém v.1.8.1');
  process.exit(1);
}
...
console.log('[OK] README.md contém cabeçalho v.1.8.1, título # Open Mark e link de acesso online imediato');
