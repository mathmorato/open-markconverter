...
if (APP_CONFIG.VERSION !== 'v.1.7.7') {
  console.error('[ERRO] Versão diferente de v.1.7.7');
  process.exit(1);
}
...
if (pkg.version !== '1.7.7' || !/^[0-9]\.[0-9]\.[0-9]$/.test(pkg.version)) {
...
if (!indexHtml.includes('v.1.7.7')) {
  console.error('[ERRO] index.html não contém v.1.7.7');
  process.exit(1);
}
...
console.log('[OK] index.html contém v.1.7.7, novo cabeçalho Open Mark com logo SVG e dropzone semântica');

// 3.1. Verifica concorrência dinâmica de 50 workers e otimização requestAnimationFrame
const configJs = fs.readFileSync('./js/config.js', 'utf8');
if (!configJs.includes('getDynamicConcurrency') || !configJs.includes('HIGH_VOLUME: 50')) {
  console.error('[ERRO] js/config.js não contém getDynamicConcurrency ou HIGH_VOLUME: 50');
  process.exit(1);
}
console.log('[OK] js/config.js contém configuração de concorrência escalável (50 workers)');

// 4. Verifica listeners, download individual, telemetria, linearização, desativação de toasts e 3 blocos no js/app.js
const appJs = fs.readFileSync('./js/app.js', 'utf8');
if (!appJs.includes('getDynamicConcurrency') || !appJs.includes('dispatchNext') || !appJs.includes('requestAnimationFrame')) {
  console.error('[ERRO] js/app.js não contém getDynamicConcurrency, dispatchNext ou requestAnimationFrame');
  process.exit(1);
}
console.log('[OK] js/app.js contém pool dinâmico de 50 workers e batching com requestAnimationFrame');
...
if (!stylesCss.includes('.format-badges-list') || !stylesCss.includes('.limit-indicator') || !stylesCss.includes('.visually-hidden')) {
  console.error('[ERRO] css/styles.css não contém estilos para .format-badges-list, .limit-indicator ou .visually-hidden');
  process.exit(1);
}
console.log('[OK] css/styles.css contém layout travado de cabeçalho (CLS=0), estilos da nova dropzone e botão estável');

// 6. Verifica README.md
const readme = fs.readFileSync('./README.md', 'utf8');
if (!readme.includes('v.1.7.7')) {
  console.error('[ERRO] README.md não contém v.1.7.7');
  process.exit(1);
}
...
console.log('[OK] README.md contém cabeçalho v.1.7.7, título # Open Mark e link de acesso online imediato');
