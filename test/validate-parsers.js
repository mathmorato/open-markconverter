...
if (APP_CONFIG.VERSION !== 'v.1.6.2') {
  console.error('[ERRO] Versão diferente de v.1.6.2');
  process.exit(1);
}
...
if (pkg.version !== '1.6.2' || !/^[0-9]\.[0-9]\.[0-9]$/.test(pkg.version)) {
...
if (!indexHtml.includes('v.1.6.2')) {
  console.error('[ERRO] index.html não contém v.1.6.2');
  process.exit(1);
}
...
console.log('[OK] index.html contém v.1.6.2, ausência de toast-container, limit-badge 1,5 GB e fila em lote');
...
if (!readme.includes('v.1.6.2')) {
  console.error('[ERRO] README.md não contém v.1.6.2');
  process.exit(1);
}
...
console.log('[OK] README.md contém cabeçalho v.1.6.2 e link de acesso online imediato');
