...
if (APP_CONFIG.VERSION !== 'v.1.8.2') {
  console.error('[ERRO] Versão diferente de v.1.8.2');
  process.exit(1);
}
...
if (pkg.version !== '1.8.2' || !/^[0-9]\.[0-9]\.[0-9]$/.test(pkg.version)) {
...
if (!indexHtml.includes('v.1.8.2')) {
  console.error('[ERRO] index.html não contém v.1.8.2');
  process.exit(1);
}
...
if (!indexHtml.includes('id="headless-mode-notice"') || !indexHtml.includes('headless-badge')) {
  console.error('[ERRO] index.html não contém o aviso de modo alto desempenho (#headless-mode-notice)');
  process.exit(1);
}
...
console.log('[OK] index.html contém v.1.8.2, novo cabeçalho Open Mark com logo SVG e barra global de progresso');
...
if (!appJs.includes('BATCH_HEADLESS_THRESHOLD') || !appJs.includes('shouldEnableHeadlessMode') || !appJs.includes('updateHeadlessBanner')) {
  console.error('[ERRO] js/app.js não contém BATCH_HEADLESS_THRESHOLD, shouldEnableHeadlessMode ou updateHeadlessBanner');
  process.exit(1);
}
console.log('[OK] js/app.js contém pool dinâmico de até 1000 workers, batchAnimationController e modo headless (>= 50 itens)');
...
if (!stylesCss.includes('.headless-mode-notice') || !stylesCss.includes('.headless-badge')) {
  console.error('[ERRO] css/styles.css não contém regras de estilo para .headless-mode-notice ou .headless-badge');
  process.exit(1);
}
console.log('[OK] css/styles.css contém layout travado de cabeçalho (CLS=0), barra com GPU e estilos do modo headless');

// 6. Verifica README.md
const readme = fs.readFileSync('./README.md', 'utf8');
if (!readme.includes('v.1.8.2')) {
  console.error('[ERRO] README.md não contém v.1.8.2');
  process.exit(1);
}
...
console.log('[OK] README.md contém cabeçalho v.1.8.2, título # Open Mark e link de acesso online imediato');
